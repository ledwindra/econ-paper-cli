...
already assume) for exactly the three platform/architecture combinations
this project's CI matrix tests: macOS arm64, Linux x86_64, and Windows
x86_64. All three are the CPU-only release asset for that platform (no
GPU/accelerator variant), matching the "no GPU requirement" product
guardrail. ``archive_size_bytes``/``archive_sha256`` were computed directly
from the real release assets downloaded from the URLs below.
"""
